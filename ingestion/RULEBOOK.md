...
2. **Convert (no AI).** `python tools/pdf_to_text.py <raw pdf> <P>/text/<id>.raw.md`
   ...
   - Drawings: use `--raw` (compact, content-stream order; layout mode pads large sheets with spaces) and expect scattered text with no visual meaning (see `doc-types/drawing.md`).
...
10. **Chunk.** `python tools/chunk_doc.py <P>/sanitized/<id>.md --type <type> --doc-id <ID> --title "<generic title>" --out <P>/kb/chunks --approved [--meta key=json ...]` For specs, `chunk_doc.py` picks the sectioning style itself (`--sections-from auto`, the default: `SECTION nn nn nn` heading lines, or the running page headers when a spec has no heading lines); force one with `--sections-from headings` or `page-headers`.
...
- **Identifiers:** contract, solicitation, PO, tender, file and project numbers; business numbers; any number that embeds a project code (in `ORG-2145-CO-007` keep only `CO-007`).
...
### 4.3 Consistency
The same real party always maps to the same placeholder within a project (numbering `-1`, `-2` by first appearance). Record every assignment in `private/mapping.csv`, **and every form in which a party appears**: the full name, initial plus surname (`S. SURNAME` in a title block), initials, abbreviations, spelled-out and short forms of a place, and variants split across lines. After each new alias, re-run the leak check (no mapped real value may remain in any sanitized file of the project). Never write the real value into any file outside `private/`.
...
- A leak check across the whole project passes: every real value and alias in `private/mapping.csv` is absent from every file in `sanitized/` and `kb/`, after the last alias was added.
- When the project ends, `raw/`, `text/` and `private/` are deleted or kept only where the organization permits (the mapping table is the key that re-identifies everything).
