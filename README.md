...
| understand the webapp's public/private split and what's still deferred | `ARCHITECTURE.md` |
...
| inflation-adjust a comparable | `data/statcan-ippi-construction.README.md` (materials cost, not a price) |
| map this kit's data to the Open Contracting Data Standard | `sources/OCDS-MAPPING.md`, `tools/ocds_export.py` |
| check the tools work | `python tools/test/run_all.py` |
| see what a real run taught us | `ingestion/TRIAL-colonel-by-drive.md` |
| use it without an AI coding assistant | `webapp/README.md` — `python webapp/server.py`, local by default |

## What is in the repository
- **The skill.** `SKILL.md` (agent instructions in the Agent Skills form), `templates/justification-template.md`, `sources/register.csv` (28 entries: 23 read at the page, API or downloaded file they cite, 1 partly verified, 1 marked unverified, 2 planning assumptions, 1 internal companion-repo reference), `sources/OCDS-MAPPING.md` (mapping to the Open Contracting Data Standard).
- **Gates.** `gates/sensitive-info-gate.md` (rules for warning about locations, vendors, consultants, contractors and individuals — scoped to raw project records; already-public dataset identifiers are explicitly exempt, see its own scope note) and `tools/scan_sensitive.py` (pattern scanner; supports reviewed exceptions with `--allow`).
...
- **Tools** (pure Python, standard library; `pdftotext` from poppler, or `pypdf`, for PDFs): `init_project.py`, `pdf_to_text.py`, `prescrub.py`, `chunk_doc.py`, `spec_headers.py`, `build_index.py`, `scan_sensitive.py`, `canadabuys_filter.py`, `statcan_ippi_filter.py`, `ocds_export.py`. Tested on Python 3.10.
- **Data.** `data/canadabuys-awards-ncr-construction.csv` (125 Ottawa-area construction awards, 2022-2026), `data/canadabuys-tender-attachments-index.csv` (links only; 555), `data/statcan-ippi-construction.csv` (Statistics Canada commodity price index, filtered to 5 construction-relevant groups, 2015-2026), their READMEs, `data/SCOUTING-canadabuys.md` (what CanadaBuys offers and its rules), `data/SOURCES-candidates.md` (survey of further public sources), and `data/comparables.csv` (empty by design). A private companion repository, `fair-price-corpus`, holds generic facts extracted from sanitized project documents once reviewed (`data/README.md`); it is not used by any tool here yet.
- **Webapp.** `webapp/` — local by default (`127.0.0.1`), and can run as a shared instance behind a per-caller token roster (`--host`, `--require-auth`; see `ARCHITECTURE.md` and `webapp/auth.py`). UI for scanning text, browsing the register and award data, looking up a commodity-index inflation adjustment, and drafting, for someone without an AI coding assistant. Stdlib only, no `pip install`; drafting is the one route that calls an LLM (see `webapp/README.md`).
- **Tests.** `tools/test/` (synthetic data only): `python tools/test/run_all.py` runs ten groups, including an end-to-end pipeline test (skipped with a message if `pdftotext` is not installed).
