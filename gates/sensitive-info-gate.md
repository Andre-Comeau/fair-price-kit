...
- Matches a human has reviewed and accepted (a public body's name, an ALL-CAPS drawing note read as an address, a printed proprietary notice) go into a project's `kb/gate-allow.txt` (`scan_sensitive.py --allow FILE`; `build_index.py` reads it automatically). Only the user adds lines; only matching text is silenced.
